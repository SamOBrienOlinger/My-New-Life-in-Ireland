...
import {
  ...
  ExternalLink, GraduationCap, HeartHandshake, Languages, Map, MapPin,
...
export default function Home() {
  ...
  return <main>
    <section className="hero-section hero-section-expanded" id="top">
      ...
      <div className="hero-content hero-content-expanded"><p className="eyebrow">An interactive journey through Ireland’s migration system</p><h1>A new country is not one decision. It is a chain of them.</h1><p className="hero-copy">Choose one of nine fictional people. Make decisions before departure, at the border and after arrival. See the consequences, learn the Irish rules and find the source behind every stage.</p><div className="hero-actions"><a className="primary-button hero-button" href="#route-finder">Find a journey <ArrowRight size={18} aria-hidden="true" /></a><a className="quiet-button hero-secondary" href="resources/"><BookOpen size={18} aria-hidden="true" />Explore practical information</a></div><div className="hero-proof" aria-label="Experience summary"><span><strong>9</strong> people</span><span><strong>5</strong> route types</span><span><strong>25</strong> decisions</span><span><strong>1</strong> Irish context</span></div></div>
    </section>

    <section className="experience-map" aria-labelledby="experience-title"><div><p className="eyebrow">How the experience works</p><h2 id="experience-title">Plan. Choose. See what changes.</h2></div><ol><li><span>01</span><strong>Enter a real-world situation</strong><p>Each fictional composite draws on common decisions people face.</p></li><li><span>02</span><strong>Choose what happens next</strong><p>Decisions reveal consequences without punishing experimentation.</p></li><li><span>03</span><strong>Learn the Irish rule</strong><p>Plain-language explanations link to current official or specialist sources.</p></li><li><span>04</span><strong>Build arrival knowledge</strong><p>Journeys continue into registration, work, study, family and community life.</p></li></ol></section>
    ...
    <section className="character-section character-section-expanded" id="choose-character">
      <div className="section-heading"><div><p className="eyebrow">Choose a perspective</p><h2>Whose journey will you take?</h2></div><div className="learning-progress-card"><Sparkles size={20} aria-hidden="true" /><div><strong>{completedCharacters.length} of 9 journeys explored</strong><span>Progress is saved only on this device.</span></div></div></div>
